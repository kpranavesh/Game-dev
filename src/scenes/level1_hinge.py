...
class Level1Hinge:
    ...
    def __init__(self, screen_w, screen_h):
        ...
        self.idx = 0
        self.card_x = screen_w + CARD_W  # start off-screen right
        ...
        # Button rects (used for both drawing and click detection)
        self.nope_rect = pygame.Rect(self.w//2 - CARD_W//2 - 70, self.h//2 + 80, 56, 56)
        self.like_rect = pygame.Rect(self.w//2 + CARD_W//2 + 14, self.h//2 + 80, 56, 56)
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if self.nope_rect.collidepoint(event.pos):
                self._do_nope()
            elif self.like_rect.collidepoint(event.pos):
                self._do_like()
    ...
    def update(self, dt):
        ...
        cx = self.target_x  # center x of card
    ...
    def _advance(self):
        ...
        if self.idx >= len(self.profiles):
            self.idx = 0  # loop (shouldn't happen with Krishna in deck)
    ...
    def _draw_card(self, surface, profile, x, y):
        ...
        pygame.draw.rect(port_surf, (0, 0, 0, 0), (0, 0, CARD_W - 20, 230), border_radius=14)
        ...
        if self.show_nope and self.nope_alpha > 0:
            stamp = self.font_big.render("NOPE", True, (*NOPE_RED, self.nope_alpha))
            stamp_surf = pygame.Surface((stamp.get_width() + 20, stamp.get_height() + 10), pygame.SRCALPHA)
            pygame.draw.rect(stamp_surf, (*NOPE_RED, min(80, self.nope_alpha)),
                             (0, 0, stamp_surf.get_width(), stamp_surf.get_height()), border_radius=8)
    ...
    def draw(self, surface):
        ...
        # Instruction hint
        if self.state == self.IDLE:
            hint = self.font_small.render("Click ✕ / ♡  or  ← / → keys", True, MID_BROWN)
        ...
        # Swipe buttons
        pygame.draw.circle(surface, NOPE_RED, self.nope_rect.center, 28)
        pygame.draw.circle(surface, LIKE_GREEN, self.like_rect.center, 28)
        x_surf = self.font_name.render("✕", True, WHITE)
        heart_surf = self.font_name.render("♡", True, WHITE)
        surface.blit(x_surf, (self.nope_rect.centerx - x_surf.get_width()//2, self.nope_rect.centery - x_surf.get_height()//2))
        surface.blit(heart_surf, (self.like_rect.centerx - heart_surf.get_width()//2, self.like_rect.centery - heart_surf.get_height()//2))
